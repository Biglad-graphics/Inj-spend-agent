...
bot.command("help", (ctx) => {
  ctx.replyWithMarkdown(`⚡ *INJ Spend Agent*\nChoose an option:`, MAIN_MENU);
});
  ...
  const task = cron.schedule(cronExpr, async () => {
    ...
    if (bal < amountInj) {
      bot.telegram.sendMessage(
        telegramId,
        `⚠️ Scheduled payment failed: not enough balance. Need ${amountInj} INJ, have ${bal.toFixed(4)} INJ.`
      );
      return;
    }
...
cron.schedule("*/5 * * * *", async () => {
  ...
  for (const alert of activeAlerts) {
    ...
    if (bal < alert.threshold_inj) {
      bot.telegram.sendMessage(
        alert.telegram_id,
        `🚨 Balance Alert! Dropped below ${alert.threshold_inj} INJ. Current: ${bal.toFixed(4)} INJ\n\nDeposit to: ${user.address}`
      );
    }
  }
});
