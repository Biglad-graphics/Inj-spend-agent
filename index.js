...
function migrateUser(user) {
  // Migrate old single-wallet users to multi-wallet format
  if (!user.wallets) {
    user.wallets = [{
      id: Date.now(),
      name: "Wallet 1",
...
// ─── PIN state ────────────────────────────────────────────────────────────────
const pinState = {};
const namingState = {}; // tracks wallet naming flow
...
bot.command("removepin", async (ctx) => {
  ...
  const user = db.users[telegramId];
  if (!user) return ctx.reply("Use /start first.");
  if (!user.pin_hash) return ctx.reply("No PIN set.");
  pinState[telegramId] = {
    action: "verify_pin", pendingAction: async () => {
      const db2 = await readDB();
      db2.users[telegramId].pin_hash = null;
      await writeDB(db2);
      ctx.reply("✅ PIN removed.");
    }
  };
  ...
  for (const r of rows) {
    await ctx.replyWithMarkdown(
      `📅 *#${r.id}* ${r.label || "Payment"}\n${r.amount_inj} INJ to \`${r.to_address.slice(0, 20)}...\`\n⏰ ${r.cron_expr}`,
      {
        reply_markup: {
          inline_keyboard: [[
            { text: "❌ Cancel", callback_data: `cancel_sched_${r.id}` },
          ]],
        },
      }
    );
  ...
  for (const r of rows) {
    await ctx.replyWithMarkdown(
      `🔔 *Alert #${r.id}*\nNotify when balance drops below *${r.threshold_inj} INJ*`,
      {
        reply_markup: {
          inline_keyboard: [[
            { text: "❌ Cancel Alert", callback_data: `cancel_alert_${r.id}` },
          ]],
        },
      }
    );
...
bot.command("cancelschedule", async (ctx) => {
  ...
  const db = await readDB();
  const s = db.schedules.find((s) => s.id === id && s.telegram_id === String(ctx.from.id));
...
bot.command("cancelalert", async (ctx) => {
  ...
  const db = await readDB();
  const a = db.alerts.find((a) => a.id === id && a.telegram_id === String(ctx.from.id));
...
bot.command("history", async (ctx) => {
  const db = await readDB();
  const telegramId = String(ctx.from.id);
  const rows = db.tx_log.filter((t) => t.telegram_id === telegramId).slice(-10).reverse();
  if (!rows.length) return ctx.reply("No transactions yet.");
  const lines = rows.map((r) => {
    const date = new Date(r.created_at).toLocaleDateString();
    return `${r.status === "success" ? "✅" : "❌"} ${r.type} | ${r.amount_inj} INJ | ${date}` +
...
bot.on("callback_query", async (ctx) => {
  ...
  // Add wallet
    ...
    if (!user) return ctx.reply("Use /start first.");
    if (user.wallets.length >= MAX_WALLETS) return ctx.reply(`Max ${MAX_WALLETS} wallets reached.`);
    namingState[telegramId] = { action: "name_new_wallet", type: "generate" };
    ctx.reply("What would you like to name this wallet?\n\nE.g. Savings, Trading, DeFi");

  // Switch wallet
  } else if (action.startsWith("switch_wallet_")) {
    const wId = parseInt(action.replace("switch_wallet_", ""));
    if (!user) return ctx.reply("Use /start first.");
    user.wallets.forEach((w) => w.active = w.id === wId);
    db.users[telegramId] = user;
    await writeDB(db);
